    ...
    \resizebox{\linewidth}{!}{
        \begin{tikzpicture} 
            ...
            \begin{axis}[
                ...
                \only<1,3,5>{
                    \addplot[ultra thick,domain=0:0.5] {sin(deg(16*pi*x))}; 
                }
            \end{axis}
            
            \only<1>{
                \foreach \p in {0,1,...,6} {
                    \coordinate (p\p) at (20*0.125/3*2*\p,{sin(deg(16*pi*0.125/3*2*\p))});
                    \fill [red, opacity=0.5] (p\p) circle (2pt);
                }
                
            }
            \only<3>{
                \foreach \p in {0,1,...,12} {
                    \coordinate (p\p) at (20*0.125/3*\p,{sin(deg(16*pi*0.125/3*\p))});
                    \fill [red, opacity=0.5] (p\p) circle (2pt);
                }
            ...
            \only<5>{
                \foreach \p in {0,1,...,12} {
                    \coordinate (p\p) at
                    ({20*0.125/3*(\p+0.3)},{sin(deg(16*pi*0.125/3*(\p+0.3)))});
                    \fill [red, opacity=0.5] (p\p) circle (2pt);
                }
            ...
            \node[label] at (11.2,0) {time (ms)};
            
            \draw (0,-2) sin (0.25,-1.75) cos (0.5,-2) sin (0.75,-2.25) cos (1,-2)
            node[label, right=0.3, black] {original signal\\(8kHz)};
            
            \only<1-2>{
                \fill [red] (4,-2) circle (2pt) node[label, right=0.3, black] {sampled point\\(12kHz)};
                \draw [dashed] (7,-2) -- ++(0.5,-0.3) -- ++(1,0.6) -- ++(0.5,-0.3)
                node[label, right=0.3, black] {resulting signal\\(4kHz)};
            }
            \only<3-6>{
                \fill [red] (4,-2) circle (2pt) node[label, right=0.3, black] {sampled point\\(24kHz)};
                \draw [dashed] (7,-2) -- ++(0.25,-0.3) -- ++(0.5,0.6) -- ++(0.5,-0.6) --
                ++(0.5,0.6) -- ++(0.25,-0.3)
                node[label, right=0.3, black] {resulting signal\\(8kHz)};
            }
        \end{tikzpicture} 
    }
  \end{center}
...
\end{tikzpicture}
    ...
    \only<2-3>{
        ...
        $\lambda = \frac{c}{f}$\hspace{1em} $D=\frac{1}{2} c\cdot t$ \hspace{1em} $t=\frac{\varphi}{2 \pi f}$
    }
                \only<2>{
        ...
        modulating frequency; $\varphi$ the (measured) phase shift.
    }
    \only<3>{

    \begin{align*}
        D &= \frac{\lambda}{4\pi}\varphi \\
        \Rightarrow D' &= L + 2 \cdot D = L + \frac{\varphi}{2 \pi} \cdot \lambda \\
    \end{align*}
    }
            \end{column}
        \end{columns}
    ...
    \only<4>{
        ...
        For $f$ = 5 MHz\bubblemark{wavelength}, $\lambda$ = 60 meters.
        ...
        \bubble<4>[-30][3cm]{wavelength}{Not the light frequency! The laser \emph{power modulation} (ie pulses) frequency}
    }
    \only<5>{
...
\begin{frame}{Structured light -- Principle}
    ...
    \begin{tikzpicture} 
        ...
        \coordinate (lens) at ($(target)+(\incidenceAngle-10:5.67)$);
        \coordinate (transmitter) at ($(target)+(180-\incidenceAngle-10:5)$);
        ...
        \path (transmitter) pic[scale=0.5,rotate=-\incidenceAngle-10]{camera};
        ...
        \draw[<->] (3.9,0.5 |- lens) -- (3.9,1 |- psd) node[label, right,midway] {$f$};
        ...
        \draw (transmitter) + (0.5,0) arc [start angle=0, end angle=-\incidenceAngle-10, radius=0.5] node[midway,right] {$\alpha$};
    ...
    \only<2>{
        ...
        \begin{columns}
            \begin{column}{0.5\linewidth}
                ...
                \resizebox{!}{0.8\paperheight}{
                    \begin{tikzpicture} 
                        ...
                        \coordinate (E) at ($(A) + (255:10)$);
                        \coordinate (H) at ($(A) + (285:10)$);
                        \coordinate (C) at ($(A) + (0,-7)$);
                        \node[anchor=south west] at (C) {C};
                        \coordinate (B) at (intersection of A--E and C--{$(C) + (1,0)$});
                        \node[anchor=south east] at (B) {B};
                        \coordinate (D) at (intersection of A--H and C--{$(C) + (1,0)$});
                        ...
                        \draw[thick, blue] (A) -- (E) node[anchor=north,black] {E};
                        \draw[thick, green] (A) -- (H) node[anchor=north, black] {H};
                        \draw[black] (A) -- (C) node[label, right,midway] {$z$};
                        \draw[>-<,thick, red,shorten <=-0.5cm, shorten >=-0.5cm] (B) -- (D);

                        \draw[|-|,thick, red] ($(F) + (-1,0)$) -- ($(F) + (1,0)$);
                        \draw[|-|,thick, red] ($(G) + (-1,0)$) -- ($(G) + (1,0)$);
                        \draw (B) -- (F);
                        \draw (D) -- (G);

                        \draw[<->] ($(D)+(0,-0.2)$) -- ($(B)+(0,-0.2)$) node[label,midway,below] {baseline $b$};
                        \draw[<->] ($(B)+(0.2,0)$) -- ($(F)+(0.2,0)$) node[label,midway, above,sloped] {focal length $f$};

                        \node[label,below=0.5 of F] {\bf Left camera};
                        \node[label,below=0.5 of G] {\bf Right camera};
                        \node[label,above=0.5 of A] {\bf Object};
            ...
            \begin{column}{0.5\linewidth}
                ...
                $k = BD \times BF = b \cdot f$\\
                $z = AC$

                \vspace{1em}

                $d = \frac{b\cdot f}{z}$ is the \textbf{disparity}
            \end{column}
        \end{columns}
...
\end{multicols}
        ...
        \only<2>{
            ...
            \video{0.8\linewidth}{figs/structured_light.mp4}
            ...
            \begin{table}[htpb]
                ...
                \begin{tabular}{ccc}
                    ...
                    Camera col. & 335 & 392 \\
                    {\bf Disparity} & 335-314=21 & 392-250=142 \\
...
\begin{frame}{Time-of-Flight cameras}
    \begin{center}
        ...
        \includegraphics<2>[width=0.6\linewidth]{tof2}


        \includegraphics<2->[width=0.6\linewidth]{tof1.pdf}

        \only<2>{
            \source{https://en.wikipedia.org/wiki/Time-of-flight_camera}{Wikipedia}
        }
