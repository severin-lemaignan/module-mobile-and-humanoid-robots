...
\textbf{constant} (for example, measuring a voltage or a temperature).
    ...
    \vspace{3em}
    \only<2-5>{$\mathbf{P}_{k\mid k-1} = \mathbf{F}\cdot \mathbf{P}_{k-1\mid k-1}\cdot \mathbf{F}^{\text{T}} + \mathbf{Q}\bubblemark[0pt]{ex1-4} \only<5>{ = \mathbf{P}_{k\mid k-1} = \mathbf{P}_{k-1\mid k-1} + \colvec{0.0001}}$}
    ...
    \bubble<4>[110][0.5][2cm]{ex1-4}{$\mathbf{Q}$?\\ $\mathbf{Q}=\begin{bmatrix}0.0001\end{bmatrix}$}
}
    \only<6-10>{
    ...
    \only<6->{$\mathbf{K}_k = \frac{\mathbf{P}_{k\mid k-1}\cdot \mathbf{H}^{\text{T}\bubblemark[0pt]{ex1-5}}}{\mathbf{H}\cdot \mathbf{P}_{k\mid k-1}\cdot \mathbf{H}^{\text{T}} + \mathbf{R}} \only<7->{= \frac{\mathbf{P}_{k\mid k-1}}{\mathbf{P}_{k\mid k-1} + \bubblemark[0pt]{ex1-6}\mathbf{R}}} \only<8->{= \frac{\mathbf{P}_{k\mid k-1}}{\mathbf{P}_{k\mid k-1} + \colvec{0.01}}}$}
...
Q = mat.matrix([0.0001]); R = mat.matrix([0.01])
...
def kalman(t):
    ...
    P.append((mat.identity(1) - K) * P_prior)
...
\only<2-3> {
    ...
    \only<4-6>{
        ...
        The trick consists in embeding the velocity in our state: $\mathbf{x} = \colvec{y \\ \dot{y}}$
        ...
        \only<6->{
            ...
            \begin{align*}
                z_k &= \mathbf{H}\cdot \colvec{y \\ \dot{y}}_{k-1\mid k-1} + \mathbf{w}_k \\
                    &= \begin{bmatrix}1 \\ 0\end{bmatrix} \cdot \begin{bmatrix}y \\ \dot{y}\end{bmatrix}_{k-1\mid k-1} + \mathbf{w}_k
            \end{align*}
