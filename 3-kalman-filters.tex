...
\begin{frame}{Round and round goes the Kalman filter}
    ...
    \bubble<1>[200][5.5][3cm]{aposteriori}{the notation $\hat{\mathbf{x}}_{n\mid m}$
    represents the estimate of $\mathbf{x}$ at time $n$ given observations up
    ...
    \begin{itemize}
        \item The \textbf{time update} step.
    ...
    \begin{tikzpicture}[>=latex]
        ...
        \node[align=center] at (-2,0) (A) {Time update\\\emph{"Predict"}};
    ...
    Notation: $\hat{\mathbf{x}}_{t\mid t-1}$
    ...
    \begin{itemize}
        \item \emph{A posteriori} because the estimate is made after new sensor
            measurements have come in.
        \item Noted $\hat{\mathbf{x}}_{t\mid t}$
    \end{itemize}
}
\end{frame}
...
\begin{frame}{Predict step equations}
    ...
    \vspace{2em}
    $\mathbf{P}_{t\mid t-1} = \mathbf{A}\cdot \mathbf{P}_{t-1\mid t-1} \mathbf{A}^{\text{T}} + \mathbf{Q}$


%    \bubble<1>[230][0.5]{up1}{A priori estimate of the state at time $k$}
%    \bubble<1>[230][0.5]{up2}{A posteriori estimate of the state at time $k-1$}
%    \bubble<1>[230][0.5]{up3}{A priori estimate of the error covariance at time $k$}
%    \bubble<1>[230][0.5]{up4}{A poseriori estimate of the error covariance at time $k-1$}
%    \bubble<1>[230][0.5]{up5}{Process noise}
