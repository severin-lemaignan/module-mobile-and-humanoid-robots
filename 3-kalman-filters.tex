...
\begin{frame}{Basic concepts: Gaussian or normal}
    ...
    \begin{columns}
        ...
        \bubblemark{cov} % dummy one that needs to appear on slides <1>
            ...
            \[
                p(x_k) = p(\begin{bmatrix}
                    x_1 \\
                    \vdots \\ 
                    x_k
                \end{bmatrix}) \sim \mathcal{N}_k(\mu, \Sigma\bubblemark[-2ex]{cov}):
            \]

        \[
            p(x_k) =
                \frac{1}{\sqrt{(2\pi)^{k}|\Sigma|}} e^{-\frac{1}{2}(x-\mu)^\mathrm{T}\Sigma^{-1}(x-\mu)}
            \]
    }
        \end{column}
    ...
    \bubble<2>{cov}{$k \times k$ covariance matrix}
...
\begin{frame}{The process equations (1)}
    ...
    \bubble<1->[230]{pe1}{The state at time step $t$}
...
\only<1>{
    If $cov(X,Y) > 0$, then $X$ and $Y$ tend move together. If
        $cov(X,Y) < 0$ then $X$ and $Y$ have an opposite effect on
...
\begin{frame}{Round and round goes the Kalman filter}
    ...
    \begin{itemize}
        \item $\hat{\mathbf{x}}_{t\mid t}$\bubblemark[-1ex]{aposteriori}: the state estimate at time $t$ given observations up to and including at time $t$
        \item $\mathbf{P}_{t\mid t}$: the \emph{error covariance matrix} (a measure of the estimated accuracy of the state estimate)
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Notation}: $\hat{\mathbf{x}}_{t\mid t-1}$
    \end{itemize}
    ...
    \begin{itemize}
        \item \emph{A posteriori} because the estimate is made after new sensor
            measurements have come in.
        \item \textbf{Notation}: $\hat{\mathbf{x}}_{t\mid t}$
    \end{itemize}
}
\end{frame}
...
\begin{frame}{Predict step equations}
    ...
    \Huge\centering
    $\hat{\mathbf{x}}_{t\mid \bubblemark[0pt]{up1}t-1} = \mathbf{A}\cdot \hat{\mathbf{x}}_{t-1\mid \bubblemark[0pt]{up2}t-1} + \mathbf{B}\cdot u_{t-1}$

    \vspace{2em}
    $\mathbf{P}_{t\mid \bubblemark[0pt]{up3}t-1} = \mathbf{A}\cdot \mathbf{P}_{t-1\mid \bubblemark[0pt]{up4}t-1}\cdot \mathbf{A}^{\text{T}} + \bubblemark[0pt]{up5}\mathbf{Q}$


    \bubble<1>[130][0.5][3cm]{up1}{A priori estimate of the state at time $t$}
    \bubble<1>[130][0.5][4cm]{up2}{A posteriori estimate of the state at time $t-1$}
    \bubble<1>[130][0.5][3cm]{up3}{A priori estimate of the error covariance at time $t$}
    \bubble<1>[100][0.5][4cm]{up4}{A poseriori estimate of the error covariance at time $t-1$}
    \bubble<1>[90][0.5][1.5cm]{up5}{Process noise}
...
\begin{frame}{Correct step equations (measurement update)}
    ...
    \Huge\centering
    $\bubblemark[0pt]{up6}\mathbf{K}_t = \frac{\mathbf{P}_{t\mid t-1}\cdot \mathbf{H}^{\text{T}}}{\mathbf{H}\cdot \mathbf{P}_{t\mid t-1}\cdot \mathbf{H}^{\text{T}} + \bubblemark[0pt]{up8}\mathbf{R}_t}$

    \vspace{2em}
    \huge
    $\hat{\mathbf{x}}_{t\mid \bubblemark[0pt]{up7}t} = \highlight<2>{\hat{\mathbf{x}}\bubblemark[-1ex]{up10}_{t\mid t-1}} + \highlight<3>{\mathbf{K}_t\cdot (\mathbf{z}_t - \mathbf{H}\bubblemark[-1ex]{up9}\cdot \hat{\mathbf{x}}_{t\mid t-1})}$

    \vspace{2em}
    $\mathbf{P}_{t|t} = (I - \mathbf{K}_t\cdot \mathbf{H})\cdot \mathbf{P}_{t|t-1}$
...
\begin{frame}{The Kalman filter loop}
    ...
    \begin{tikzpicture}[>=latex]
        ...
        \node[align=center,text width=5cm,anchor=north] at (-3.5,0) (A) {\emph{Prediction}\small
                                                            \begin{enumerate}
                                                                \item Project the state ahead \\
                                                                $\hat{\mathbf{x}}_{t\mid t-1} = \mathbf{A}\cdot \hat{\mathbf{x}}_{t-1\mid t-1} + \mathbf{B}\cdot u_{t-1}$
                                                                \item Project the error covariance ahead \\
                                                                $\mathbf{P}_{t\mid t-1} = \mathbf{A}\cdot \mathbf{P}_{t-1\mid t-1}\cdot \mathbf{A}^{\text{T}} + \mathbf{Q}$
                                                        \end{enumerate}
                                                            };
        \node[align=center,text width=6cm,anchor=north] at (2.5,0) (B) {\emph{Correction}\small
                                            \begin{enumerate}
                                                \item Compute Kalman gain \\
                                                $\mathbf{K}_t = \mathbf{P}_{t\mid t-1}\cdot \mathbf{H}^{\text{T}}(\mathbf{H}\cdot \mathbf{P}_{t\mid t-1}\cdot \mathbf{H}^{\text{T}} + \mathbf{R}_t)^{-1}$
                                                \item Update estimate with measurement $z_t$ \\
                                                $\hat{\mathbf{x}}_{t\mid t} = \hat{\mathbf{x}}_{t\mid t-1} + \mathbf{K}_t\cdot (\mathbf{z}_t - \mathbf{H}\cdot \hat{\mathbf{x}}_{t\mid t-1})$
                                                \item Update the error covariance \\
                                                $\mathbf{P}_{t|t} = (I - \mathbf{K}_t\cdot \mathbf{H})\cdot \mathbf{P}_{t|t-1}$
                                            \end{enumerate}
        };
        \node[align=center,text width=4cm,anchor=north] at (-4,-5) (C) {\emph{Initial estimates} for $\hat{\mathbf{x}}_{t-1}$ and $\mathbf{P}_{t-1}$};
