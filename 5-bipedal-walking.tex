...
\videoframe[0.56]{figs/part5/hughherr-bionic-limbs.mp4}
    ...
    \textbf{Just because it's cool!}
        ...
        \begin{column}{0.5\linewidth}
            \begin{center}
                ...
                \video[0.56]{\linewidth}{figs/part5/cyberdyne-hal.mp4}
            \end{center}
        \end{column}
    \end{columns}
...
\begin{frame}{What is bipedal walking?}
    ...
    \only<1>{
        ...
        \textbf{Walking}: the two feet are simultaneously in contact with the ground at
        some point during the gait cycle.

        \textbf{Running}: one foot at maximum touches the ground at any point in the
        gait cycle.
    ...
    \only<2>{
        ...
        \begin{center}
            \includegraphics[width=0.8\linewidth]{gait-phases}
        \end{center}
    }
...
\begin{frame}{Bioloid forward kinematics}
    ...
    \only<2>{
        How does that generalize to more degrees of freedom (DoF)?
    }
...
\begin{frame}{Bioloid Gait Analysis}
    ...
    In order to reduce the computational cost to calculate the equation
    of an ellipse every 15.6ms, the solution is pre-calculated over a grid
