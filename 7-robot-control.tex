...
\begin{tikzpicture}[
 ...
 \uncover<+->{
  ...
  \path (lowlevel.east) edge [<-, bend right=80, looseness=1.5] node[label] {primitive\\actions} (shary.east);
 }
 ...
 \uncover<+->{
  ...
  \draw[dotted, thick] (dialogs.west |- mid) -- (mhp.east |- mid) node[at start,above,align=left,draw=none] {\bf Deliberative/executive layers} 
                                                                  node[at start,below,align=left,draw=none] {\bf Functional layer};
...
\begin{frame}{Middlewares' core principle: Talking Nodes}
    ...
    \only<1>{
        \textbf{Nodes} are software components which realise a function (can be a
         simple behaviour, a data filter, a complex FSM, or anything else really)
    }
